...
public sealed class GoapAgent : MonoBehaviour, IAgent
{
    ...
    // Previously: private void Update()
    public void Tick(float deltaTime)
    {
        stateMachine.Update(gameObject, deltaTime);
    }
    ...
    {
        idleState = (fsm, gameObj, deltaTime) =>
        {
            // GOAP planning
    ...
    {
        moveToState = (fsm, gameObj, deltaTime) =>
        {
            // move the game object

            var action = currentActions.Peek();
            if (action.requiresInRange() && !action.hasTarget())
            {
                Debug.Log(
            ...
            if (dataProvider.moveAgent(action, deltaTime))
            {
                fsm.popState();
            }
    ...
    {
        performActionState = (fsm, gameObj, deltaTime) =>
        {
            // perform the action
